...
class EventRepository {
  ...
  Future<AppEvent> createEvent(AppEvent event, String currentUserId) async {
    try {
      ...
      final newEvent = event.copyWith(
        id: eventId,
        userId: int.parse(currentUserId), // 🔹 Asignar el userId del usuario actual
        createdAt: now,
        updatedAt: now,
      );
      ...
      await NotificationService.scheduleEventNotification(newEvent, currentUserId: int.parse(currentUserId));
  ...
  Future<AppEvent> updateEvent(AppEvent event, String currentUserId) async {
    try {
      ...
      await NotificationService.scheduleEventNotification(updatedEvent, currentUserId: int.parse(currentUserId));
